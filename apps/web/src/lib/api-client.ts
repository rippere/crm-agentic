...
export const apiClient = {
  ...
  getDealsByAgent: (workspaceId: string, token: string): Promise<Array<{ agent_name: string; count: number; total_value: number }>> => {
    if (isDemoMode) {
      return Promise.resolve([
        { agent_name: 'Nova',       count: 5, total_value: 520000 },
        { agent_name: 'Sage',       count: 3, total_value: 310000 },
        { agent_name: 'Unassigned', count: 2, total_value: 95000  },
      ])
    }
  ...
  getContactLastTouch: (
    ...
    if (isDemoMode) {
      const stubs: Record<string, { last_message_date: string | null; last_note_date: string | null; last_activity_date: string | null; most_recent_type: string | null; days_ago: number | null }> = {
        'c-001': { last_message_date: '2026-06-25', last_note_date: '2026-06-28', last_activity_date: '2026-06-29', most_recent_type: 'activity', days_ago: 1 },
        'c-002': { last_message_date: '2026-06-10', last_note_date: '2026-06-15', last_activity_date: null, most_recent_type: 'note', days_ago: 15 },
        'c-003': { last_message_date: '2026-05-20', last_note_date: null, last_activity_date: null, most_recent_type: 'message', days_ago: 41 },
      }
  ...
  getDealRevenueForecast: (workspaceId: string, token: string): Promise<Array<{ month: string; expected_revenue: number; deal_count: number; total_value: number }>> => {
    if (isDemoMode) {
      return Promise.resolve([
        { month: '2026-07', expected_revenue: 125000, deal_count: 3, total_value: 230000 },
        { month: '2026-08', expected_revenue: 89000,  deal_count: 2, total_value: 185000 },
        { month: '2026-09', expected_revenue: 215000, deal_count: 4, total_value: 420000 },
      ])
    }
    return apiFetch(`/workspaces/${workspaceId}/deals/revenue-forecast`, {}, token)
  },

  getDealStageAging: (workspaceId: string, token: string): Promise<Array<{ id: string; title: string | null; company: string | null; stage: string; value: number; days_in_stage: number }>> => {
    if (isDemoMode) {
      return Promise.resolve([
        { id: 'd-001', title: 'TechCorp Expansion',          company: 'TechCorp',    stage: 'discovery',   value: 145000, days_in_stage: 52 },
        { id: 'd-004', title: 'StartupBase Pilot',           company: 'StartupBase', stage: 'discovery',   value: 32000,  days_in_stage: 8  },
        { id: 'd-005', title: 'Nexus AI Enterprise',         company: 'Nexus AI',    stage: 'qualified',   value: 110000, days_in_stage: 21 },
        { id: 'd-002', title: 'Global Finance Suite',        company: 'GlobalFin',   stage: 'proposal',    value: 250000, days_in_stage: 38 },
        { id: 'd-003', title: 'Accelerate Partners Renewal', company: 'Accelerate',  stage: 'negotiation', value: 88000,  days_in_stage: 14 },
        { id: 'd-006', title: 'MediaHub Platform',           company: 'MediaHub',    stage: 'negotiation', value: 65000,  days_in_stage: 7  },
      ])
    }
    return apiFetch(`/workspaces/${workspaceId}/deals/stage-aging`, {}, token)
  },

  getDealWinProbabilityByStage: (workspaceId: string, token: string): Promise<Array<{ stage: string; avg_probability: number; deal_count: number; total_value: number }>> => {
    if (isDemoMode) {
      return Promise.resolve([
        { stage: 'discovery',   avg_probability: 22.0, deal_count: 3, total_value: 287000 },
        { stage: 'qualified',   avg_probability: 45.0, deal_count: 2, total_value: 195000 },
        { stage: 'proposal',    avg_probability: 63.0, deal_count: 2, total_value: 395000 },
        { stage: 'negotiation', avg_probability: 78.0, deal_count: 2, total_value: 233000 },
      ])
    }
  ...
  getContactPipelineContribution: (
    workspaceId: string,
    token: string,
  ): Promise<Array<{ contact_id: string; name: string | null; email: string | null; company: string | null; pipeline_value: number; closed_won_value: number; deal_count: number; win_rate: number }>> => {
    if (isDemoMode) {
      return Promise.resolve([
        { contact_id: 'c-002', name: 'Marcus Rivera',   email: 'marcus.r@globalfin.com',    company: 'GlobalFin',   pipeline_value: 250000, closed_won_value: 0,      deal_count: 1, win_rate: 0    },
        { contact_id: 'c-001', name: 'Sarah Chen',      email: 'sarah.chen@techcorp.com',   company: 'TechCorp',    pipeline_value: 145000, closed_won_value: 95000,  deal_count: 3, win_rate: 66.7 },
        { contact_id: 'c-005', name: 'Devon Park',      email: 'devon@nexusai.io',          company: 'Nexus AI',    pipeline_value: 110000, closed_won_value: 0,      deal_count: 1, win_rate: 0    },
        { contact_id: 'c-004', name: 'James Whitfield', email: 'jwhitfield@accelerate.com', company: 'Accelerate',  pipeline_value: 88000,  closed_won_value: 180000, deal_count: 2, win_rate: 50   },
      ])
    }
  ...
  getDealConcentrationRisk: (
    workspaceId: string,
    token: string,
  ): Promise<{ total_pipeline: number; top_deals: Array<{ id: string; title: string | null; company: string | null; stage: string; value: number; pct_of_pipeline: number }>; top3_pct: number; risk_level: 'low' | 'medium' | 'high' }> => {
    if (isDemoMode) {
      return Promise.resolve({
        total_pipeline: 690000,
        top3_pct: 71.0,
        risk_level: 'high' as const,
        top_deals: [
          { id: 'd-002', title: 'Global Finance Suite',        company: 'GlobalFin',  stage: 'proposal',    value: 250000, pct_of_pipeline: 36.2 },
          { id: 'd-001', title: 'TechCorp Expansion',          company: 'TechCorp',   stage: 'discovery',   value: 145000, pct_of_pipeline: 21.0 },
          { id: 'd-005', title: 'Nexus AI Enterprise',         company: 'Nexus AI',   stage: 'qualified',   value: 110000, pct_of_pipeline: 15.9 },
          { id: 'd-003', title: 'Accelerate Partners Renewal', company: 'Accelerate', stage: 'negotiation', value: 88000,  pct_of_pipeline: 12.8 },
          { id: 'd-006', title: 'MediaHub Platform',           company: 'MediaHub',   stage: 'negotiation', value: 65000,  pct_of_pipeline: 9.4  },
        ],
      })
    }
  ...
  getDealCloseDateAccuracy: (
    workspaceId: string,
    token: string,
  ): Promise<Array<{ id: string; title: string | null; company: string | null; value: number; expected_close: string; actual_close: string; days_delta: number; outcome: 'early' | 'on_time' | 'late' }>> => {
    if (isDemoMode) {
      return Promise.resolve([
        { id: 'd-w1', title: 'MediaHub Phase 2',    company: 'MediaHub',    value: 65000,  expected_close: '2026-04-01', actual_close: '2026-04-22', days_delta: 21,  outcome: 'late'    },
        { id: 'd-w2', title: 'Nexus AI Renewal',    company: 'Nexus AI',    value: 95000,  expected_close: '2026-03-15', actual_close: '2026-03-22', days_delta: 7,   outcome: 'late'    },
        { id: 'd-w3', title: 'StartupBase License', company: 'StartupBase', value: 60000,  expected_close: '2026-02-28', actual_close: '2026-02-28', days_delta: 0,   outcome: 'on_time' },
        { id: 'd-w4', title: 'Accelerate Renewal',  company: 'Accelerate',  value: 88000,  expected_close: '2026-05-20', actual_close: '2026-05-17', days_delta: -3,  outcome: 'early'   },
        { id: 'd-w5', title: 'TechCorp Q1 Close',   company: 'TechCorp',    value: 145000, expected_close: '2026-05-15', actual_close: '2026-04-30', days_delta: -15, outcome: 'early'   },
      ])
    }
    return apiFetch(`/workspaces/${workspaceId}/deals/close-date-accuracy`, {}, token)
  },

  getActivityTrends: (workspaceId: string, token: string, weeks = 12): Promise<Array<{ week_start: string; total: number; deals: number; contacts: number; agents: number; messages: number }>> => {
    if (isDemoMode) {
      const now = new Date()
      const monday = new Date(now)
      ...
      for (let i = weeks - 1; i >= 0; i--) {
        const ws = new Date(monday)
        ws.setDate(monday.getDate() - i * 7)
        const seed = Math.floor(ws.getTime() / 86400000)
        const deals    = (seed * 7  + 3) % 8
        const contacts = (seed * 11 + 5) % 6
        const agents   = (seed * 13 + 1) % 5
        const messages = (seed * 17 + 2) % 9
        result.push({
          week_start: ws.toISOString().slice(0, 10),
          total: deals + contacts + agents + messages,
          deals,
          contacts,
          agents,
          messages,
  ...
  getContactReengagementSummary: (workspaceId: string, token: string, weeks = 12): Promise<Array<{ week_start: string; reengaged: number }>> => {
    if (isDemoMode) {
      ...
      monday.setDate(now.getDate() - now.getDay() + 1)
      monday.setHours(0, 0, 0, 0)
      const pattern = [0, 1, 0, 2, 1, 3, 0, 1, 2, 1, 0, 2]
      const result = []
      for (let i = weeks - 1; i >= 0; i--) {
        const ws = new Date(monday)
        ws.setDate(monday.getDate() - i * 7)
